...
We first use Dynamic World to identify Mongolia’s forest–grassland–desert transition zone and select a central-southern AOI. We then create land-cover composites for 2016–2018 and 2021–2023, compare them to detect changed pixels, and classify key transitions into degradation and recovery. These pixel-level results are aggregated into 10 km grid cells, where we calculate change, degradation, recovery, and net change indicators. The grid dataset is then used to identify observed hotspots. Following the driver framework of Meng et al. (2021), climate, terrain, and soil variables are extracted and overlaid onto the same grid to train a Random Forest model, so the app can show both current degradation patterns and areas of higher concern.
...
-   **Static Driver Assumption**: The RF model uses environmental conditions from the early period only, rather than incorporating how those conditions changed between periods. This means the model captures where degradation is likely based on existing conditions, but may miss cases where a sudden shift triggered localised degradation.
